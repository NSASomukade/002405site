...
if (currentIndex > -1) {
  ...
  if (postDateFormat.test(postsArray[currentIndex][0].slice(6, 17))) {
    let monthSlice = postsArray[currentIndex][0].slice(6, 8);
    let month = "";
    if (monthSlice === "01") {
      month = "Jan";
    ...
    niceDate = postsArray[currentIndex][0].slice(9, 11) + " " + month + ", " + postsArray[currentIndex][0].slice(12, 16);
  }
}
...
function formatPostLink(i) {
  ...
  if (postDateFormat.test(postsArray[i][0].slice(6, 17))) {
    return '<li><a href="' + relativePath + "/" + postsArray[i][0] + '">' + "\u005B" + postsArray[i][0].slice(6, 16).replace(/-/g, ".") + "\u005D " + postTitle_i + "</a></li>";
  } else {
...
if (postsArray.length > recentPostsCutoff) {
  recentPostListHTML += '<ul><li class="moreposts"><a href=' + relativePath + "/archive.html>\u00BB more posts</a></li></ul>";
} else {
  recentPostListHTML += " ";
}
